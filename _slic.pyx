                ...
                for x in range(width):
                    ...
                    for i in range(6):
                        ...
                        if (0 <= xx < width and
                                    0 <= yy < height and
                                    0 <= zz < depth):
                            if nearest_segments[zz, yy, xx] != label:
                                change = 1
                                for ii in range(0, top[label] + 1):
                                    if(adj[label,ii] == nearest_segments[zz, yy, xx]):
                                        change = 0
                                        break
                                if change == 1:
                                    adj[label,top[label]] = nearest_segments[zz, yy, xx]
                                    top[label] += 1
                                    
    return np.asarray(nearest_segments), np.asarray(segments), np.asarray(adj[:k+1,:])
...
def _enforce_label_connectivity_cython(Py_ssize_t[:, :, ::1] segments,
    ...
    cdef Py_ssize_t depth, height, width
    cdef double[:, ::1] center_new = np.ones_like(center)
    ...
    cdef Py_ssize_t zz, yy, xx, z, y, x
    ...
    with nogil:
                        ...
                        for i in range(6):
                            ...
                            if (0 <= xx < width and
                                ...
                                if (segments[zz, yy, xx] == label and
                                        connected_segments[zz, yy, xx] == -1): #zzyyxx的label和zyx的label相同  且  zzyyxx新label没
                                    connected_segments[zz, yy, xx] = \
                                        current_new_label
                                    center_new[current_new_label] = center[label]
        ...
        for z in range(depth):
            for y in range(height):
                for x in range(width):
                    label = connected_segments[z, y, x]
                    for i in range(6):
                        zz = z + ddz[i]
                        yy = y + ddy[i]
                        xx = x + ddx[i]
                        if (0 <= xx < width and
                                    0 <= yy < height and
                                    0 <= zz < depth):
                            if connected_segments[zz, yy, xx] != label:
                                change = 1
                                for ii in range(0, top[label] + 1):
                                    if(adj[label,ii] == connected_segments[zz, yy, xx]):
                                        change = 0
                                        break
                                if change == 1:
                                    adj[label,top[label]] = connected_segments[zz, yy, xx]
                                    top[label] += 1
    return np.asarray(connected_segments), np.asarray(center_new), np.asarray(adj[:current_new_label,:])
